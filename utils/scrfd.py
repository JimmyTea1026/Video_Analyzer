...
class SCRFD:
    ...
    def prepare(self, ctx_id, **kwargs):
        if ctx_id<0:
            self.session.set_providers(['GPUExecutionProvider'])
    ...
    def forward(self, img, threshold):
        ...
        fmc = self.fmc
        check = {0.6:False, 0.7:False, 0.8:False, 0.9:False}
        ...
        return scores_list, bboxes_list, kpss_list, check
    ...
    def detect(self, img, input_size = None, thresh=None, max_num=0, metric='default'):
        ...
        scores_list, bboxes_list, kpss_list, check = self.forward(det_img, det_thresh)
    ...
    def autodetect(self, img, max_num=0, metric='max'):
        ...
        bboxes, kpss, check1 = self.detect(img, input_size=(640, 640), thresh=threshold)
        bboxes2, kpss2, check2 = self.detect(img, input_size=(128, 128), thresh=threshold)
        check = {}
        for key, item in check1.items():
            check[key] = check1[key] or check2[key]
